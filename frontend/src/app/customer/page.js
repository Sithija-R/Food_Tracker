...
export default function CustomerDashboard() {
  ...
  const placeOrder = async () => {
    if (!orderDetails.description || !orderDetails.lat || !orderDetails.lng || !orderDetails.address) {
    ...
    <div className="min-h-screen bg-gray-100 p-6 bg-gradient-to-b from-sky-300 to-sky-100">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-3 items-center">
          ...
          <button onClick={handleLogout} className="bg-sky-900 text-white px-4 py-2 rounded-3xl">
            Logout
          </button>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="mt-6">
            ...
            <input
              ...
              onChange={(e) => setOrderDetails({ ...orderDetails, lat: e.target.value })}
              className="border p-2 w-full mb-2 rounded"
            />
            <input
              type="number"
              ...
              onChange={(e) => setOrderDetails({ ...orderDetails, lng: e.target.value })}
              className="border p-2 w-full mb-2 rounded"
            />
            <input
              type="text"
          ...
          {orders.map((order) => (
            <div key={order.id} className="bg-white p-4 rounded mb-4">
              ...
              {order.customerLocation && (
                <p>
                  Location: {order.customerLocation.address} ({order.customerLocation.lat}, {order.customerLocation.lng})
                </p>
              )}
            </div>
          ))}
